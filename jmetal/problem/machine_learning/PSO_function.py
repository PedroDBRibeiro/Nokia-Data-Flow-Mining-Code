    ...
    while j < iter_max:
        ...
        for p in swarm:
            ...
            for clf in p[0].keys():
                if clf == 'LogReg':
                    ## C
                    v = w * p[2] + c1 * np.random.uniform(0, 1) * (
                                p[3][clf][0] - p[0][clf][0]) + c2 * np.random.uniform(0,
                                                                                      1) * (
                                gbest[3][clf][0] - p[3][clf][0])
                    p[0][clf][0] = abs(p[0][clf][0] + v)
                    ## max_iter
                    v = w * p[2] + c1 * np.random.uniform(0, 1) * (
                                p[3][clf][1] - p[0][clf][1]) + c2 * np.random.uniform(0,
                                                                                      1) * (
                                gbest[3][clf][1] - p[3][clf][1])
                    p[0][clf][1] = abs(p[0][clf][1] + int(v))
                elif clf == 'RF':
                    ## n_estimarors (RF)
                    v = w * p[2] + c1 * np.random.uniform(0, 1) * (p[3][clf] - p[0][clf]) + c2 * np.random.uniform(0,
                                                                                                                   1) * (
                                gbest[3][clf] - p[3][clf])
...
if __name__ == '__main__':
    print("reading data...")
    MLData = getData("/home/phnarloch/Área de Trabalho/data/original_PSOdata.csv")
    y = MLData.getY()
    X = MLData.getX()
    ...
    horizontal_stack.to_csv("Unbalanced" + "_validationDataSet.csv", header=True)

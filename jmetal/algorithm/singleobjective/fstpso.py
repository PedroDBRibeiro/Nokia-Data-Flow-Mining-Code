...
import subprocess
import pickle
from jmetal.core.solution import Particle
...
class PSO_new(object):
    ...
    def calc_median_sum(self, j):
        ...
        for y in self.Solutions:
            final_pop.append(y.X)
        dim_median = list(np.mean(final_pop, axis=0))
        total_sum = 0
        for p in final_pop:
            total_sum += dim_median[j] - p[j]
        total_sum = abs(total_sum)
        total = (1 / len(final_pop)) * total_sum
    ...
    def UpdateDiversity(self):
        ...
        self.calc_diversity(len(self.Solutions[0].X))

        if self.Iterations == 1:  # First Generation
    ...
    def Solve(self, funz, verbose=False, callback=None, dump_best_fitness=None, dump_best_solution=None,
        ...
        if self._discrete_cases is None:
            return self.G, self.G.CalculatedFitness, self.G.attributes[0]
        else:
...
class PSO_j(ParticleSwarmOptimization):
    def __repr__(self):
        return str("<PSO instance " + self.ID + ">")
...
class FuzzyPSO(PSO_new):
    ...
    def solve_with_fstpso(self,
                          ...
                          dump_final_swarm=None,dump_diversity=None):
        """
        ...
        return self._actually_launch_optimization(verbose=verbose, callback=callback,
                                                  ...
                                                  dump_final_swarm = dump_final_swarm, dump_diversity=dump_diversity)

    def _actually_launch_optimization(self, verbose=None, callback=None, dump_best_solution=None,
                                      dump_best_fitness=None, dump_final_swarm = None, dump_diversity = None):
        if verbose:
        ...
        result = self.Solve(None, verbose=verbose, callback=callback, dump_best_solution=dump_best_solution,
                            dump_best_fitness=dump_best_fitness)
        with open(dump_final_swarm, "a") as fo:
            for i in range(len(self.Solutions)):
                fo.write("\t".join(map(str, self.Solutions[i].X)) + "\n")

        with open(dump_diversity+'FSTPSO_xpt.csv', "a") as fo:
            fo.write(",".join(map(str, self.xpt_history)) + "\n")
        with open(dump_diversity+'FSTPSO_xpl.csv', "a") as fo:
            fo.write(",".join(map(str, self.xpl_history)) + "\n")
    ...
    def call_fitness(self, particle, arguments=None):
        if self.FITNESS is None:
            raise Exception("ERROR: fitness function not valid")

        data = particle.X
        particle.variables = particle.X

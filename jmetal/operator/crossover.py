...
class DifferentialEvolutionCrossover(Crossover[FloatSolution, FloatSolution]):
    ...
    def __init__(self, CR: float, F: float, K: float, probability: float = 0.0):
    ...
    def execute(self, parents: np.ndarray) -> FloatSolution:
        """ Execute the differential evolution crossover ('rand/1/bin' variant in jMetal)."""
        ...
        for i in range(number_of_variables):
            if random.random() < self.CR or i == rand:
                ...
                value = self.boundary_correction.correct_boundaries(child.upper_bound[i],
                                                                    child.lower_bound[i],
...
class DifferentialEvolutionCurrentCrossover(DifferentialEvolutionCrossover):
    """ This operator receives two parameters: the current individual and an array of three parent individuals. The
    ...
    def __init__(self, CR: float, F: float, K: float):
        super().__init__(CR, F, K)
        self.parameters_dict = {}

    def execute(self, parents: List[FloatSolution]) -> FloatSolution:
        ...
        for i in range(number_of_variables):
            if random.random() < self.CR or i == rand:
                value = self.current_individual.variables[i] + \
                        self.F * (parents[2].variables[i] - self.current_individual.variables[i]) + \
                        self.F * (parents[0].variables[i] - parents[1].variables[i])

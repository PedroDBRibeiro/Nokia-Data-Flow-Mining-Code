...
class DifferentialEvolutionSelection(Selection[List[S], List[S]]):
    ...
    def __init__(self, mating_size: int = 3):
        super().__init__()
        self.index_to_exclude = None
        self.mating_size = mating_size

    def execute(self, front: List[S]) -> np.ndarray:
        if front is None:
            raise Exception('The front is null')
        elif len(front) == 0:
            raise Exception('The front is empty')
        ...
        while self.index_to_exclude in selected_indexes:
            selected_indexes = np.random.choice(len(front), 3)

        mating = np.array([front[_] for _ in selected_indexes])
...
class PBestSelectionWithArchive(Selection):

    def __init__(self):
        super().__init__()
        self.index_to_exclude = None

    def execute(self, source: [List[S], List[S]]) -> np.ndarray:
        population = source[0]
        archive = source[1]
        pmin = 2.0 / len(population)
        p = np.random.rand() * (0.2 - pmin) + pmin
        max_best = int(p * len(population))
        best_solutions = np.argsort([_.objectives[0] for _ in population])[:max_best]
        ...
        while 1:
            ...
            if r1 != self.index_to_exclude:
                break

        population_individual = population[r1]

        if len(archive) == 0:
            while 1:
                r2 = np.random.randint(0, len(population))
                if r2 != self.index_to_exclude and r2 != r1:
                    break
            archive_individual = population[r2]
        else:
            ...
            while 1:
                ...
                if r2 != self.index_to_exclude and r2 != r1:
                    break
            archive_individual = pop_archive[r2]

        mating = np.array([xbest, population_individual, archive_individual])
    ...
    def get_name(self) -> str:
        return "pBest with Archive"


class PBestSelectionWithoutArchive(Selection):
    ...
    def execute(self, front: List[S]) -> np.ndarray:
        pmin = 2.0 / len(front)
        p = np.random.rand() * (0.2 - pmin) + pmin
        max_best = int(p * len(front))
        best_solutions = np.argsort([_.objectives[0] for _ in front])[:max_best]

        pbest = np.random.choice(best_solutions)

        xbest = front[pbest]

        selected_indexes = random.sample(range(len(front)), 2)

        while self.index_to_exclude in selected_indexes:
            selected_indexes = np.random.choice(len(front), 2)

        mating = np.array([xbest] + [front[_] for _ in selected_indexes])
    ...
    def get_name(self) -> str:
        return "pBest without Archive"


class DifferentialEvolutionBestMinSelection(DifferentialEvolutionSelection):

    def __init__(self):
        super(DifferentialEvolutionBestMinSelection, self).__init__()
        self.index_to_exclude = []
    ...
    def execute(self, front: List[S]) -> List[S]:
        ...
        best_index = np.argsort([_.objectives[0] for _ in front])[0]
        self.index_to_exclude.append(best_index)

        selected_indexes = random.sample(range(len(front)), 2)
        while self.index_to_exclude in selected_indexes:
            selected_indexes = random.sample(range(len(front)), 2)

        selected_indexes = selected_indexes + [best_index]

        return [front[i] for i in selected_indexes]

    def get_best_solution_index(self, solutions: List[S]) -> int:
        best_index = 0

        for i, solution in enumerate(solutions):
            if solution.objectives[0] < solutions[best_index].objectives[0]:
                best_index = i
    ...
    def set_index_to_exclude(self, index: int):
        self.index_to_exclude = []
        self.index_to_exclude.append(index)

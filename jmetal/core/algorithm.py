...
from jmetal.algorithm.data.diversity import DimensionWiseDiversity
...
class Algorithm(Generic[S, R], threading.Thread, ABC):
    ...
    def __init__(self):
        ...
        #self.xpl = 0
        #self.xpt = 0 
        #self.diversity_calculator = DimensionWiseDiversity()
...
class EvolutionaryAlgorithm(Algorithm[S, R], ABC):
    ...
    def step(self):
        #print(len(self.solutions[0].variables[0]))
        #self.diversity_calculator.update(self.evaluations, self.solutions, 1 )
        #self.xpl, self.xpt = self.diversity_calculator.get_data()
